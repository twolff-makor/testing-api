...
async function tradeFlow(numOfOtc) {
      for (let i = 0; i < numOfOtc; i++) {
        ...
        console.log(baseDelta.toString());
        console.log(quoteDelta.toString());
        ...
        if (side == "BUY") {
            if (baseDelta.isEqualTo(qty) && quoteDelta.isEqualTo(quoteAmount)) {
              ...
              console.log(`balance is not correct, side: ${side}, qty: ${qty}, base delta: ${baseDelta}, quote delta: ${quoteDelta}, quote amount: ${quoteAmount}`);
            }
        } else if (side == "SELL") {
